...
@app.route('/')
def index():
    return "http://127.0.0.1:5001/api/object_detection"
...
def object_detection():
    ...
    if 'image' in request.files:
        ...
        classes, output_layers = load_yolo()
        image, height, width, channels = load_image(imagePath)
        blob = cv2.dnn.blobFromImage(image, scalefactor=0.00392, size=(416, 416), mean=(0, 0, 0), swapRB=True,
                                     crop=False)
